...
# Test a simple query (use </dev/null in non-interactive shells — see troubleshooting)
esearch -db pubmed -query "test" </dev/null | head -20
```
...
#### 5. Pipeline “hangs” or never returns (non-interactive / CI / IDE)

EDirect’s `ecommon.sh` calls `ParseStdin`: when **standard input is not a terminal**, it runs `cat` and waits until stdin closes. In CI, some IDE tasks, or background jobs, stdin is often an **open non-TTY** stream with no data, so the tool **blocks forever**.

**Fix:** Close stdin for steps that do not intentionally pipe an `ENTREZ_DIRECT` message into the tool:

```bash
esearch -db pubmed -query "crispr" </dev/null | efetch -format xml | ...
```

Apply `</dev/null` only to the **first** command in a pipeline (usually `esearch`). Do **not** add it to `efetch` when it must read the XML stream from `esearch`.

**Related:** If stdin is a TTY but you still see long pauses, check for **`429 Too Many Requests`** in stderr (see below).

#### 6. Slow runs, `429 Too Many Requests`, or repeated retries

NCBI EUtils rate-limits clients. Without an API key you get about **3 requests/second**; with **`NCBI_API_KEY`** about **10/second** (see [API Keys](https://ncbiinsights.ncbi.nlm.nih.gov/2017/11/02/new-api-keys-for-the-e-utilities/)).

1. Create a key under NCBI Account → **API Key Management**.
2. Export it (and use a real contact email):
...
3. Avoid launching **many EDirect pipelines in parallel** from scripts or agents.
4. Optional: if connections to NCBI are flaky, try forcing IPv4: `export NQUIRE_IPV4=1`

EDirect’s `nquire` already spaces requests; heavy parallel use still triggers **429**, after which built-in retries add **sleep** and make runs look “stuck”.
...
#### 8. `esearch` hangs on FTP / version check, or Python cannot verify NCBI SSL

Some EDirect builds contact **FTP** during startup or version checks; on locked-down networks this can **time out** before any search runs. **Python** clients may also fail TLS verification against NCBI depending on the runtime CA bundle.

**Workaround (no EDirect, no Python):** use the skill’s **`scripts/`** helpers. They call **E-utilities** at `https://eutils.ncbi.nlm.nih.gov/` with **`curl`** only (`lib_eutils_curl.sh`). Example:

```bash
cd /path/to/pubmed-edirect/scripts
./search_export_csv.sh "your query [TIAB]" 50 out.csv
```

Set `NCBI_API_KEY` and `NCBI_EMAIL` as in §6 for rate limits and responsible use.

## Performance Tuning

### 1. `nquire` / curl connect timeout (seconds)

The HTTP helper honors **`NQUIRE_TIMEOUT`** (default **20**). Example:

```bash
export NQUIRE_TIMEOUT=60
```

### 2. Optional: slower pacing (debug only)

If **`SLOW_EDIRECT`** is set to a “true” value, EDirect uses **~1 second** pauses between EUtils calls instead of short micro-sleeps—pipelines become **much** slower. Unset it unless you intentionally enabled it:

---
name: pubmed-edirect
description: Search and retrieve literature from PubMed via NCBI E-utilities (HTTPS + curl in bundled scripts). EDirect is optional for interactive pipes in the docs.
requires:
  bins:
    - curl
install:
  - id: edirect
    kind: manual
    label: Optional — EDirect for interactive esearch/efetch pipelines; see INSTALL.md
    docs: https://www.ncbi.nlm.nih.gov/books/NBK179288/
    note: "⚠️ Official installer uses FTP; if blocked, use scripts/ (curl + HTTPS only)."
...
metadata:
  openclaw:
    ...
    env:
      ...
      - name: NCBI_TOOL
        optional: true
        description: Tool name sent to E-utilities (default pubmed-edirect-skill)
---

# PubMed EDirect Skill

Search and retrieve literature from PubMed. **Bundled `scripts/`** call [NCBI E-utilities](https://www.ncbi.nlm.nih.gov/books/NBK25501/) over **HTTPS with `curl`** (no local `esearch` / `efetch`, so no EDirect FTP version-check timeouts and no Python HTTPS). Interactive examples in the markdown guides still use **EDirect** when installed.

## ⚠️ Security Advisory

**Important**: Optional **EDirect** installation involves:
...
2. **Manually install EDirect**:
   ...
   ./install-edirect.sh
   ```
3. **Verify** (with EDirect installed):
   ```bash
   esearch -db pubmed -query "test" -retmax 1
   ```
   Or verify **curl-only** scripts (no EDirect):
   ```bash
   ./scripts/search_export_csv.sh "test [TIAB]" 1 /tmp/verify.csv
   ```
...
### `lib_eutils_curl.sh` / `medline_batch_to_tsv.awk`

Shared helpers: `esearch.fcgi` / `efetch.fcgi` on `https://eutils.ncbi.nlm.nih.gov/`.

### `batch_fetch_abstracts.sh`

Fetch abstracts for a list of PMIDs with error handling and rate limiting (HTTPS `efetch` via `curl`).
...
Search PubMed and export results to CSV with metadata (`esearch` + `efetch` MEDLINE over HTTPS, parsed with `awk` — no `xtract`).
...
- **FTP / `esearch` timeouts:** Local EDirect may contact FTP for version checks and hang on strict networks. Prefer **`scripts/*.sh`**, which use **E-utilities HTTPS + `curl`** only.
- **Python SSL errors:** Do not rely on Python `urllib` for NCBI in constrained environments; the scripts use **`curl`** instead.
- **Non-TTY stdin (EDirect only):** When using installed `esearch`, run `esearch … </dev/null` if you are not piping an `ENTREZ_DIRECT` document; otherwise EDirect can block on `cat` waiting for EOF.
- **Rate limits:** Set `NCBI_API_KEY` (and `NCBI_EMAIL`, optional `NCBI_TOOL`) for higher quotas; avoid many parallel jobs. See [INSTALL.md](INSTALL.md) troubleshooting.
